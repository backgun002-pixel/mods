...
public class WindReaper extends WeaponSkillItem {
    ...
    private static final List<WeaponSkill> SKILLS = List.of(
            ...
            public void execute(Player player, ItemStack stack, SkillContext ctx) {
                ...
                for (int deg = -60; deg <= 60; deg += 12) {
                    ...
                    for (int dist = 1; dist <= 4; dist++) {
                        sl.sendParticles(ParticleTypes.SWEEP_ATTACK, player.getX()+fx*dist, player.getY()+1, player.getZ()+fz*dist, 2, 0.1,0.1,0.1,0.02);
                        sl.sendParticles(ParticleTypes.CLOUD, player.getX()+fx*dist, player.getY()+1, player.getZ()+fz*dist, 1, 0.1,0.1,0.1,0.01);
                    }
                }
                player.level().playLocalSound(player.getX(), player.getY(), player.getZ(), net.minecraft.sounds.SoundEvents.PLAYER_ATTACK_SWEEP, net.minecraft.sounds.SoundSource.PLAYERS, 1.2f, 0.7f, false);
        ...
        new WeaponSkill() {
            ...
            public void execute(Player player, ItemStack stack, SkillContext ctx) {
                ...
                for (int i = 0; i < 5; i++) {
                    sl.sendParticles(ParticleTypes.CLOUD, player.getX()+look.x*i*0.8, player.getY()+1, player.getZ()+look.z*i*0.8, 8, 0.2,0.2,0.2,0.04);
                    sl.sendParticles(ParticleTypes.SWEEP_ATTACK, player.getX()+look.x*i*0.8, player.getY()+0.5, player.getZ()+look.z*i*0.8, 3, 0.1,0.1,0.1,0.02);
                }
                double tx = player.getX() + look.x * 5, tz = player.getZ() + look.z * 5;
                ...
                player.level().playLocalSound(tx, player.getY(), tz, net.minecraft.sounds.SoundEvents.PLAYER_ATTACK_STRONG, net.minecraft.sounds.SoundSource.PLAYERS, 1.5f, 0.6f, false);
        ...
        new WeaponSkill() {
            ...
            public void execute(Player player, ItemStack stack, SkillContext ctx) {
                ...
                for (LivingEntity e : sl.getEntitiesOfClass(LivingEntity.class, area)) {
                    if (e == player || !(e instanceof Monster)) continue;
                    if (e.distanceTo(player) > 5.0) continue;
                ...
                for (int ring = 0; ring < 3; ring++) {
                    ...
                    for (int i = 0; i < 36; i++) {
                        ...
                        double px = cx + Math.cos(angle)*radius, pz = cz + Math.sin(angle)*radius, py = cy + 0.5 + ring*0.4;
                ...
                for (int i = 0; i < 20; i++) {
                    ...
                    sl.sendParticles(ParticleTypes.CLOUD, cx+Math.cos(a)*r, cy, cz+Math.sin(a)*r, 3, 0.1,1.5,0.1,0.02);
                }
                player.level().playLocalSound(cx, cy, cz, net.minecraft.sounds.SoundEvents.PLAYER_ATTACK_SWEEP, net.minecraft.sounds.SoundSource.PLAYERS, 2.0f, 0.5f, false);
                player.level().playLocalSound(cx, cy, cz, net.minecraft.sounds.SoundEvents.RAVAGER_ROAR, net.minecraft.sounds.SoundSource.PLAYERS, 0.8f, 1.2f, false);
                player.displayClientMessage(Component.literal("§a§l🌪 대선풍! §f" + hit + "명 피해"), true);
            }
        },
    ...
    public WindReaper(Properties props) { super(props, JobClass.WARRIOR, 12); }
