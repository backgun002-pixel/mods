...
public class DungeonBuilder {
    ...
    public void build() {
        ...
        buildCombatRoom(r1x, y, z, ROOM_W, ROOM_H, ROOM_L, 1); z += ROOM_L;
        buildCorridor(x, y, z, CORRIDOR_L); z += CORRIDOR_L;
        int r2x = x - ROOM_W/2 + CORRIDOR_W/2;
        buildCombatRoom(r2x, y, z, ROOM_W, ROOM_H, ROOM_L, 2); z += ROOM_L;
        buildCorridor(x, y, z, CORRIDOR_L); z += CORRIDOR_L;
        int r3x = x - ROOM_W/2 + CORRIDOR_W/2;
        buildSculptureRoom(r3x, y, z, ROOM_W, ROOM_H, ROOM_L); z += ROOM_L;
        buildCorridor(x, y, z, CORRIDOR_L); z += CORRIDOR_L;
        int mbx = x - MINIBOSS_W/2 + CORRIDOR_W/2;
        buildMinibossRoom(mbx, y, z); z += MINIBOSS_L;
        buildCorridor(x, y, z, CORRIDOR_L); z += CORRIDOR_L;
    ...
    private void buildCorridor(int x, int y, int z, int length) {
        ...
        set(x-hw-1,y+2,z+length/2,DungeonBlocks.IRON_BARS); set(x+hw+1,y+2,z+length/2,DungeonBlocks.IRON_BARS);
    }
    ...
    private void buildCombatRoom(int x, int y, int z, int w, int h, int l, int roomIdx) {
        ...
        DungeonGimmickHandler.COMBAT_ROOMS.add(new DungeonGimmickHandler.RoomData(roomBounds,doors,spawnCenter,roomIdx));
    ...
    private void openDoorway(int cx, int y, int z, int height) {
        for (int dy=1;dy<=height;dy++) for (int dx=-1;dx<=1;dx++) set(cx+dx,y+dy,z,DungeonBlocks.AIR);
    }
    ...
    private void buildMinibossRoom(int x, int y, int z) {
        ...
        for (int dx=-2;dx<=2;dx++) for (int dz=-2;dz<=2;dz++) if (Math.abs(dx)==2||Math.abs(dz)==2) for (int dy=1;dy<=3;dy++) set(cx+dx,y+dy,cz+dz,DungeonBlocks.IRON_BARS);
        set(cx,y+1,cz,DungeonBlocks.SPAWNER);
        set(x+2,y+1,z+l-3,DungeonBlocks.CHEST); set(x+3,y+1,z+l-3,DungeonBlocks.CHEST);
        ...
        DungeonGimmickHandler.COMBAT_ROOMS.add(new DungeonGimmickHandler.RoomData(mbBounds,mbDoors,mbCenter,4));
        DungeonGimmickHandler.MINIBOSS_ROOMS.add(new DungeonGimmickHandler.RoomData(mbBounds,mbDoors,mbCenter,4));
        openDoorway(x+w/2,y,z,4); openDoorway(x+w/2,y,z+l-1,4);
    }

    private void buildBossRoom(int x, int y, int z) {
        int w=BOSS_W,h=BOSS_H,l=BOSS_L;
        for (int dx=0;dx<w;dx++) for (int dz=0;dz<l;dz++) { boolean edge=dx<2||dx>=w-2||dz<2||dz>=l-2; boolean diag=(dx+dz)%4==0; set(x+dx,y,z+dz,edge?DungeonBlocks.BOSS_PILLAR:diag?DungeonBlocks.BOSS_ACCENT:DungeonBlocks.BOSS_FLOOR); }
        for (int dx=0;dx<w;dx++) for (int dz=0;dz<l;dz++) { boolean center=Math.abs(dx-w/2)<5&&Math.abs(dz-l/2)<5; set(x+dx,y+h,z+dz,center?DungeonBlocks.SHROOMLIGHT:DungeonBlocks.BOSS_WALL); }
        for (int dy=1;dy<h;dy++) { BlockState wall=dy%4==0?DungeonBlocks.BOSS_ACCENT:DungeonBlocks.BOSS_WALL; for (int dx=0;dx<w;dx++) { set(x+dx,y+dy,z,wall); set(x+dx,y+dy,z+l-1,wall); } for (int dz=0;dz<l;dz++) { set(x,y+dy,z+dz,wall); set(x+w-1,y+dy,z+dz,wall); } }
        ...
        for (int pz:pillarZ) { buildBossPillar(x+1,y,pz,h); buildBossPillar(x+w-2,y,pz,h); }
        ...
        for (int dy=1;dy<=5;dy++) for (int dx=-2;dx<=2;dx++) bossDoors.add(new BlockPos(x+w/2+dx,y+dy,z+l-2));
        DungeonGimmickHandler.BOSS_DOORS=bossDoors;
        List<BlockPos> chests=new ArrayList<>();
        for (int i=0;i<4;i++) { BlockPos cp=new BlockPos(x+4+i*3,y+2,z+l-5); set(cp.getX(),cp.getY(),cp.getZ(),DungeonBlocks.CHEST); set(cp.getX(),cp.getY()+1,cp.getZ(),DungeonBlocks.IRON_BARS); chests.add(cp); }
    ...
    private void buildPillar(int x,int y,int z,int height) {
        for (int dy=0;dy<=height;dy++) set(x,y+dy,z,dy==0||dy==height?DungeonBlocks.PILLAR_TOP:DungeonBlocks.PILLAR);
    }

    private void buildGrandPillar(int x,int y,int z,int height) {
        for (int dy=0;dy<=height;dy++) { BlockState s=(dy==0||dy==height)?DungeonBlocks.WALL_CHISELED:(dy%3==0)?DungeonBlocks.ACCENT_POLY:DungeonBlocks.PILLAR; set(x,y+dy,z,s); set(x+1,y+dy,z,DungeonBlocks.PILLAR); set(x,y+dy,z+1,DungeonBlocks.PILLAR); set(x+1,y+dy,z+1,DungeonBlocks.PILLAR); }
    }

    private void buildBossPillar(int x,int y,int z,int height) {
        for (int dy=0;dy<=height;dy++) { BlockState s=(dy%3==0)?DungeonBlocks.BOSS_ACCENT:DungeonBlocks.BOSS_PILLAR; set(x,y+dy,z,s); }
        set(x,y+1,z,DungeonBlocks.BOSS_ACCENT); set(x,y+height-1,z,DungeonBlocks.BOSS_ACCENT);
        set(x,y+height-2,z+1,DungeonBlocks.LANTERN);
    }

    private void set(int x,int y,int z,BlockState state) { level.setBlock(new BlockPos(x,y,z),state,3); }
}
