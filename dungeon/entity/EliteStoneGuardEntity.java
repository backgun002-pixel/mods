...
public class EliteStoneGuardEntity extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
            .add(Attributes.MAX_HEALTH,120.0).add(Attributes.ATTACK_DAMAGE,12.0)
            .add(Attributes.ARMOR,8.0).add(Attributes.MOVEMENT_SPEED,0.22)
            .add(Attributes.KNOCKBACK_RESISTANCE,0.7).add(Attributes.FOLLOW_RANGE,28.0);
    }
    ...
    public void tick() {
        ...
        if(!enraged && this.getHealth()<this.getMaxHealth()*0.3f){
            ...
            this.level().playSound(null,this.blockPosition(),SoundEvents.RAVAGER_ROAR,this.getSoundSource(),1.2f,0.8f);
        ...
        if(chargeTick==0 && this.getTarget() instanceof Player target){
            ...
            if(dist>3.0&&dist<12.0){performCharge(target);chargeTick=CHARGE_COOLDOWN;}
        }
    }
    ...
    private void performCharge(Player target){
        ...
        this.level().playSound(null,this.blockPosition(),SoundEvents.RAVAGER_STEP,this.getSoundSource(),1.0f,1.0f);
    ...
    public void die(DamageSource source) {
        ...
        this.level().playSound(null,this.blockPosition(),SoundEvents.STONE_BREAK,this.getSoundSource(),1.5f,0.5f);
    }
