...
public class StoneGolemEntity extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
            .add(Attributes.MAX_HEALTH,70.0).add(Attributes.ATTACK_DAMAGE,10.0)
            .add(Attributes.ARMOR,10.0).add(Attributes.MOVEMENT_SPEED,0.12)
            .add(Attributes.KNOCKBACK_RESISTANCE,0.9).add(Attributes.FOLLOW_RANGE,20.0);
    }
    ...
    public void tick() {
        ...
        if(quakeTick==0 && this.getTarget() instanceof Player && this.distanceTo(this.getTarget())<4.0){
            performQuake(); quakeTick=QUAKE_COOLDOWN;
        }
    }
    ...
    private void performQuake() {
        ...
        for(Player player:this.level().getEntitiesOfClass(Player.class,area)){
            double dx=player.getX()-this.getX(), dz=player.getZ()-this.getZ();
            double len=Math.sqrt(dx*dx+dz*dz);
            if(len>0){player.push(dx/len*1.2,0.4,dz/len*1.2);player.hurt(this.damageSources().mobAttack(this),6.0f);}
        }
        this.level().playSound(null,this.blockPosition(),SoundEvents.GENERIC_EXPLODE.value(),this.getSoundSource(),0.6f,0.5f);
    }

    @Override
    public void die(DamageSource source) {
        if(this.level() instanceof ServerLevel sl) DungeonDropTable.onMonsterDrop(this,sl);
        super.die(source);
    }
